...
export default function FeedbackScreen({ onClose, onSuccess, onError }: FeedbackScreenProps) {
  ...
  const handleSubmit = async () => {
    if (!canSubmit) {
      if (!moduleValid) {
        onClose();
        // Defer error display so el modal se cierra primero (patrón Screen-level iOS)
        setTimeout(() => onError('Selecciona el módulo al que se refiere tu feedback.'), 100);
        return;
      }
      if (trimmedMessage.length < MIN_MESSAGE) {
        onClose();
        setTimeout(() => onError(`El mensaje debe tener al menos ${MIN_MESSAGE} caracteres.`), 100);
        return;
      }
      if (trimmedMessage.length > MAX_MESSAGE) {
        onClose();
        setTimeout(() => onError(`El mensaje no puede exceder ${MAX_MESSAGE} caracteres.`), 100);
        return;
      }
      return;
    }
    ...
    try {
      ...
      // Cerrar formulario PRIMERO, después mostrar error en el padre
      onClose();
      setTimeout(() => {
        if (isRateLimit) {
          onError('Has enviado mucho feedback. Intenta de nuevo en 1 hora.');
        } else if (apiMessage) {
          onError(apiMessage);
        } else {
          onError('No pudimos enviar tu feedback. Verifica tu conexión e intenta de nuevo.');
        }
      }, 100);
    } finally {
      setLoading(false);
      ...
      <KeyboardAvoidingView style={{ flex: 1 }} behavior="padding">
        ...
        >
            ...
            {TYPE_OPTIONS.map((opt) => {
              ...
              return (
                <TouchableOpacity
                  ...
                  activeOpacity={0.7}
                >
                  <Ionicons name={opt.icon} size={18} color={selected ? '#fff' : opt.color} />
          ...
          <Text style={styles.privacyNote}>
            ...
            v{Constants.expoConfig?.version ?? '?'} • iOS
          </Text>
        </ScrollView>
      </KeyboardAvoidingView>

      {/* Module Dropdown — View overlay (NO nested Modal por iOS limitation) */}
      {showModuleDropdown && (
